...
# this is to insert chosp tag only to where the initial hyper tokens are incorrect
...
def dc_set(dc, word_pair_set):
    with open(dc, 'r', encoding='utf-8') as dc_file:
        for line in dc_file:
            tokens = line.split()
            # only load the lines with 2 and different tokens
            if len(tokens) == 2 and tokens[0] != tokens[1]:
                word_pair_set[tokens[0]] = tokens[1]
...
def check_src_tgt_token(src_total_list, tgt_total_list, tgt_hpyer_total_list, dc_s, dc_list, stoplist, line_number):
    ...
    tgt_hpyer_line = tgt_hpyer_total_list[line_number]
    src_tokens = src_line.split()
    tgt_tokens = tgt_line.split()
    tgt_tokens_hyper = tgt_hpyer_line.split()
    cho_tokens = [-1, -1, '', -1, '']
    for i in range(len(src_tokens)):
        ...
        if src_tokens.count(src_tokens[i]) != 1:
            continue
        # TODO to be faster: generate both a list and a set of dc, using the set to decide if the key is in the dic and
        #  using the list to find the exact value to match the tgt sentence;
        if src_tokens[i].lower() in dc_s:
            for word_pair in dc_list:
                ...
                if cho_tokens[3] >= 0:  # success in tgt sentence
                    if not (tgt_tokens[cho_tokens[3]] in tgt_tokens_hyper):
...
def main(para):
    ...
    source_txt_wr = source_txt_rd + str(int(ratio * 100)) + para[4] + '-aim-' + para[5]

    # path for target text(before processing)
    target_txt_rd = './' + para[2]
    # path for target text(after processing)
    target_txt_wr = target_txt_rd + str(int(ratio * 100)) + para[4] + '-aim-' + para[5]

    # path for target hyper text(before processing)
    target_hyper_txt_rd = para[6]
    ...
    src_total_list = ['']
    tgt_total_list = ['']
    tgt_hyper_total_list = ['']
    file_to_list(source_txt_rd, src_total_list)
    file_to_list(target_txt_rd, tgt_total_list)
    file_to_list(target_hyper_txt_rd, tgt_hyper_total_list)
    src_total_list.pop(0)
    tgt_total_list.pop(0)
    tgt_hyper_total_list.pop(0)
    if len(src_total_list) != len(tgt_total_list) or len(src_total_list) != len(tgt_hyper_total_list):
    ...
    for i in range(total_lines):
        ...
        if len(src_total_list[i]) > 4096:
            ...
            continue
        # print('line number:', i)
        cho_tokens = check_src_tgt_token(src_total_list, tgt_total_list, tgt_hyper_total_list, dc_s, dc_list, stoplist, i)
