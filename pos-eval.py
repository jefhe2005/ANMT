...
def check_sentence_pair(line_src, line_tgt, tag):
    if tag == 'POS':
        # format in src: open_tag src_token <TO> tgt_token close_tag
        ...
        # correct = result_list.count(1); 2(partial correct) if tag is correct, but token inside has changed
        # no_match = result_list.count(9)
        src_tag = False
        tgt_tag = False
        if open_pos_src >= 0 and clos_pos_src == (open_pos_src + 4):
            src_tag = True
        if open_pos_tgt >= 0 or clos_pos_tgt >= 0:
            tgt_tag = True
        ...
        if src_tag == True and tgt_tag == True:
            print('mistake: tag shows at target side')
            return 2  #
        return -1
...
def main():
    ...
    source_txt_rd = './UNv1.0.testset.enpos'
    ...
    with open(source_txt_rd, 'r', encoding='utf-8') as source_file, open(target_txt_rd, 'r',
        ...
        for line_src in source_file:
            ...
            # print(line_src)
            # print(line_tgt)
            result_list[i] = check_sentence_pair(line_src, line_tgt, 'POS')
            i += 1
