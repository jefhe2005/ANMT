...
def check_sentence_pair(line_src, line_tgt, tag):
    if tag == 'CHOOSE':
        ...
        src_tag = False
        tgt_tag = False
        if open_pos_src >= 0 and clos_pos_src == (open_pos_src + 6):
            src_tag = True
        ...
        if src_tag == True and tgt_tag == True:
            if src_tokens[open_pos_src + 3] == tgt_tokens[open_pos_tgt + 1] or src_tokens[open_pos_src + 5] == tgt_tokens[open_pos_tgt + 1]:
                return 1  # yyy or zzz = ccc --> correct
            else:
...
def main():
    ...
    with open(source_txt_rd, 'r', encoding='utf-8') as source_file, open(target_txt_rd, 'r',
        ...
        for line_src in source_file:
            ...
            # print(line_src)
            # print(line_tgt)
            result_list[i] = check_sentence_pair(line_src, line_tgt, 'CHOOSE')
            i += 1
