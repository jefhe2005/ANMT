...
def process_file(read_file, write_file, token_list, total_process_lines):
    line_number = token_list[0][0]
    token = token_list[0][1]
    i = 0
    j = 0
        ...
        for line in source_file:
            if j < total_process_lines:
                if i == line_number:
                    ...
                    line = insert_tag(line, 'COPY', token)
                    j += 1
                    if j < total_process_lines:
                        line_number = token_list[j][0]
...
def main():
    ...
    source_txt_rd = './news-commentary-v12.zh-en.en'
    # path for source text(after processing)
    source_txt_wr = './news-commentary-v12.zh-en.en2'

    # path for target text(before processing)
    target_txt_rd = './news-commentary-v12.zh-en.zh'
    # path for target text(after processing)
    target_txt_wr = './news-commentary-v12.zh-en.zh2'
    ...
    random_line(total_lines, total_process_lines, token_list)
    ...
    random_token(source_txt_rd, token_list, total_process_lines)
    print('line number with real tokens')
    print(token_list)

    # change the line number in the token list to be random
    random_line(total_lines, total_process_lines, token_list)
    # sort again with the new line number
    token_list.sort(key=lambda x: x[0])
    print('before inserting : line number with real tokens')
    print(token_list)

    # process ( insert tags) the source and target text file
    # process the source text
    process_file(source_txt_rd, source_txt_wr, token_list, total_process_lines)
    # process the target text if the file path is not empty
    if len(target_txt_rd) > 3:
        process_file(target_txt_rd, target_txt_wr, token_list, total_process_lines)
